...
- Core installation does not require administrator privileges. On CPC Dev Boxes,
  the Hibernation Delay Agent bootstrap may request elevation.
...
# --- Step 4b: Initialize Dev Box Hibernation Delay Agent ---
if ((Get-Command praestoclaw -ErrorAction SilentlyContinue) -and ($env:COMPUTERNAME -like "CPC-*")) {
    Write-Step "Initializing Dev Box Hibernation Delay Agent ..."
    & praestoclaw hibernation-delay-agent
    if ($LASTEXITCODE -ne 0) {
        Write-Warn "Hibernation Delay Agent setup did not complete. Retry with: praestoclaw hibernation-delay-agent"
    }
}
...
if ((Get-Command praestoclaw -ErrorAction SilentlyContinue) -and (-not $skipPost)) {
    ...
    # If a prior dev/staging session leaked $env:PRAESTOCLAW_TEAMS_PACKAGE that
    # points at a path which no longer exists (e.g. a C:\src\... staging zip
    # the end-user never had), drop it so the CLI falls through to the
    # published https://aka.ms/praestoclaw/teams-app.zip download instead of
    # erroring out with "Package not found: C:\...".
    # Use -LiteralPath: the env var can be inherited from arbitrary shells and
    # may legitimately contain PowerShell wildcard chars ([, ], *, ?), which
    # plain Test-Path would mis-treat as glob patterns.
    ...
    # ``--force`` makes the one-click flow re-upload the Teams app every time
    # the installer runs. Without it users who already sideloaded an older
    # PraestoClaw release would get "already installed" and never receive
    # manifest / bot updates from a new release.
    & praestoclaw teams install --force
    if ($LASTEXITCODE -ne 0) {
